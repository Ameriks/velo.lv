...
def create_csv_seb(user=None):
    ...
    this_year_participates = set(obj.get('email') for obj in Participant.objects.filter(is_participating=True, competition_id__in=(38, 39)).exclude(email='').values('email').annotate(c=Count('id')).order_by('-c'))
    this_year_applications = set(obj.get('email') for obj in Application.objects.filter(participant__is_participating=True, competition_id__in=(38, 39)).exclude(email='').values('email').annotate(c=Count('id')).order_by('-c'))
    this_year = this_year_participates.union(this_year_applications)


    this_year_not_payed = set(obj.get('email') for obj in Participant.objects.filter(is_participating=False, competition_id__in=(38, 39)).exclude(email='').exclude(email__in=this_year).values('email').annotate(c=Count('id')).order_by('-c'))
...
this_year_participates: <a href="{0}{4}">{0}{4}</a><br>
    """.format(settings.MY_DEFAULT_DOMAIN, obj1.doc.url, obj2.doc.url, obj3.doc.url, obj.doc.url)
