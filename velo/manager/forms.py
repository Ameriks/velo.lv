...
class ParticipantIneseCreateForm(RequestKwargModelFormMixin, CleanSSNMixin, CleanEmailMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if competition.get_root().id == 1 and competition.level == 1:
            ...
            prices = [(price.id, "%s€- %s" % (str(price.price * comps.filter(is_individual=False).count() * (100 - competition.complex_discount) / 100), price.distance)) for price in prices_obj]
        else:
...
class ParticipantForm(CleanEmailMixin, RequestKwargModelFormMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if competition.get_root().id == 1 and self.instance.competition.level == 1:
            ...
            prices_obj = comps[0].price_set.all()
            prices = [(price.id, "%s€- %s" % (str(price.price * comps.filter(is_individual=False).count() * (100 - self.instance.competition.complex_discount) / 100), price.distance)) for price in prices_obj]
        else:
            prices = [(str(price.id), "%s€ - %s" % (str(price.price), price.distance)) for price in competition.price_set.all().select_related('distance')]

        if self.instance.price_id and dict(prices).get(self.instance.price_id) is None:
