...
def create_start_list(competition=None, competition_id=None):
    ...
    for distance in distances:
        ...
        row = 4
        header_row = (
            '#', 'UID', 'Numurs', 'Alias', 'Sacensības', 'Distance', 'Uzvārds', 'Vārds', 'Dzimšanas diena', 'Dzimums', 'Sacenšas?',
        ...
        for index, res in enumerate(helperresults, start=1):
            ...
            row_values = (
                ...
                item.first_name, item.birthday.strftime("%Y-%m-%d"), item.gender, item.is_competing, item.group, donation, total_entry_fee, total_insurance_fee, final_price,
