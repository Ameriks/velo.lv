    ...
    for distance in distances:
        ...
        for index, res in enumerate(helperresults, start=1):
            ...
            row_values = (
                ...
                item.first_name, item.birthday.strftime("%Y-%m-%d"), item.gender, str(item.city), str(item.is_competing), item.group, donation, total_entry_fee, total_insurance_fee, final_price,
