...
class NewsMixin(SetCompetitionContextMixin):
    def get_context_data(self, **kwargs):
        ...
        context.update({'latest_posts': News.objects.filter(published_on__lte=timezone.now()).order_by('-published_on')[:4]})

        competition_ids_with_news = News.objects.exclude(competition=None).order_by('competition_id').values('competition_id').annotate(Count('id')).values_list('competition_id')

        context.update({'competitions': Competition.objects.filter(id__in=competition_ids_with_news).order_by('-competition_date')})
